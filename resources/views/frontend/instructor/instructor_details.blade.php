        ...
        <div class="breadcrumb-content">
            <div class="media media-card align-items-center pb-4">
                <div class="media-img media--img media-img-md rounded-full">
                    <img class="rounded-full" src="{{ (!empty($instructor->photo)) ? Storage::url('public/upload/instructor_images/'. $instructor->photo) : asset('storage/upload/images.jpg') }}" alt="Student thumbnail image">
                </div>
                <div class="media-body">
                    <h2 class="section__title fs-30">{{ $instructor->name }}</h2>
                    <span class="d-block lh-18 pt-1 pb-2">Joined {{ $instructor->created_at->diffForHumans() }}</span>
            ...
            <div class="row">
                ...
                <div class="col-lg-4 responsive-column-half">
                    <div class="counter-item">
                        ...
                        <h4 class="counter__title counter text-color-4 fs-35">{{ $instructor->courses->count() }}</h4>
            ...
            <div class="tab-content pt-40px" id="myTabContent">
                <div class="tab-pane fade show active" id="about-me" role="tabpanel" aria-labelledby="about-me-tab">
                    <div class="card card-item">
                        <div class="card-body">
                            <p class="card-text pb-3">{{ $instructor->bio }}</p>
...
<section class="course-area section-padding">
    <div class="container">
        <div class="d-flex align-items-center justify-content-between pb-3">
            ...
            <span class="ribbon ribbon-lg">{{ $instructor->courses->count() }}</span>
        </div>
        <div class="divider"><span></span></div>
        <div class="row pt-30px">
            @foreach ($instructor->courses->where('status', 1)->take(6) as $course)
            <div class="col-lg-4 responsive-column-half">
    ...
    <div id="tooltip_content_1{{ $course->id }}">
        <div class="card card-item">
            <div class="card-body">
                ...
                <h5 class="card-title pb-1"><a href="{{ route('course_details', ['id' => $course->id, 'slug' => $course->slug]) }}">{{ $course->name }}</a></h5>
                <div class="d-flex align-items-center pb-1">
                    @if ($course->best_seller == 1)
                    <h6 class="ribbon fs-14 mr-2 orange">Bestseller</h6>
                    @endif
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <a href="{{ route('course_details', ['id' => $course->id, 'slug' => $course->slug]) }}" class="btn theme-btn flex-grow-1 mr-3">Go to course</a>
                    @auth
                        <div class="icon-element icon-element-sm shadow-sm cursor-pointer wishlist-icon" title="Add to Wishlist" id="{{ $course->id }}" onclick="addToWishlist(this.id)">
                            @if (auth()->user()->wishlistCourses->contains($course->id))
                                <i class="la la-heart icon-red"></i></div>
