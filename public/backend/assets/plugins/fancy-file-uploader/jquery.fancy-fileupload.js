...
(function($) {
	...
	var DisplayPreviewDialog = function(preview, endelem, inforow, data, settings) {
		...
		var previewdialog = $('<div>').addClass('ff_fileupload_dialog_main');
	
		// Create a new <img> element and set its src attribute to the URL of the file
		var previewImage = $('<img>').attr('src', previewclone.attr('src'));
		previewdialog.append(previewImage);
	...
	$.fn.FancyFileUpload = function(options) {
		...
		// var AddFile = function(uploads, e, data, settings) {
		// 	// Clear the table or the specific section of the table where the files are listed
		// 	uploads.empty(); // This line clears the table
		...
		// 			// Set the background image of the preview button to the image file
		// 			var previewImage = inforow.find('.ff_fileupload_preview_image');
		// 			previewImage.css('background-image', 'url(' + e.target.result + ')');
		
		// 			// Add click event to display the preview dialog
		// 			previewImage.click(function(e) {
		// 				e.preventDefault();
		// 				DisplayPreviewDialog(previewImage, this, inforow, data, settings);
		// 			});
		// 		};
		// 		reader.readAsDataURL(data.files[0]); // Read the file as a Data URL
		// 	}
		
		// 	// Display file name and size
		// 	inforow.find('.ff_fileupload_filename').text(data.files[0].name);
		// 	inforow.find('.ff_fileupload_fileinfo').text(data.files[0].size + ' bytes');
		...
		// 		inforow.remove();
		// 	};
		// 	inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', 'Remove from list').click(RemoveFile));
		
		// 	// Append the new file to the table
		...
		document.addEventListener('DOMContentLoaded', function() {
			var uploads = $('.ff_fileupload_uploads');
			var currentImage = $('#old_image').val();
			console.log(currentImage);
		
			if (currentImage) {
				var inforow = $('<tr><td class="ff_fileupload_preview"><button class="ff_fileupload_preview_image" type="button"><span class="ff_fileupload_preview_text"></span></button><div class="ff_fileupload_actions_mobile"></div></td><td class="ff_fileupload_summary"><div class="ff_fileupload_filename"></div><div class="ff_fileupload_fileinfo"></div><div class="ff_fileupload_buttoninfo ff_fileupload_hidden"></div><div class="ff_fileupload_errors ff_fileupload_hidden"></div><div class="ff_fileupload_progress_background ff_fileupload_hidden"><div class="ff_fileupload_progress_bar"></div></div></td><td class="ff_fileupload_actions"></td></tr>');
		
				inforow.find('.ff_fileupload_preview_image').css('background-image', 'url(' + currentImage + ')');
				inforow.find('.ff_fileupload_filename').text(currentImage.split('/').pop());
				inforow.find('.ff_fileupload_fileinfo').text('Current image');
				...
				// Add click event to display the preview dialog
				inforow.find('.ff_fileupload_preview_image').click(function() {
					// Create a new <img> element and set its src attribute to the URL of the file
					var imgElement = $('<img>').attr('src', currentImage);
		
					// Call DisplayPreviewDialog with the imgElement as the preview
					// Since we're in the edit scenario, we don't have 'data' and 'settings' available.
					// You might need to adjust these parameters based on your application's needs.
					DisplayPreviewDialog(imgElement, this, inforow, null, null);
				});
			...
			$('#image').on('change', function(e) {
				...
				if (file) {
					...
					reader.onload = function(e) {
						var inforow = $('<tr><td class="ff_fileupload_preview"><button class="ff_fileupload_preview_image" type="button"><span class="ff_fileupload_preview_text"></span></button><div class="ff_fileupload_actions_mobile"></div></td><td class="ff_fileupload_summary"><div class="ff_fileupload_filename"></div><div class="ff_fileupload_fileinfo"></div><div class="ff_fileupload_buttoninfo ff_fileupload_hidden"></div><div class="ff_fileupload_errors ff_fileupload_hidden"></div><div class="ff_fileupload_progress_background ff_fileupload_hidden"><div class="ff_fileupload_progress_bar"></div></div></td><td class="ff_fileupload_actions"></td></tr>');
						inforow.find('.ff_fileupload_preview_image').css('background-image', 'url(' + e.target.result + ')');
						inforow.find('.ff_fileupload_filename').text(file.name);
						inforow.find('.ff_fileupload_fileinfo').text(file.size + ' bytes');
						...
						inforow.find('.ff_fileupload_preview_image').click(function() {
							...
							// Call DisplayPreviewDialog with the imgElement as the preview
							// Since we're in the add scenario, we don't have 'data' and 'settings' available.
							// You might need to adjust these parameters based on your application's needs.
							DisplayPreviewDialog(imgElement, this, inforow, null, null);
						});
		...
		// 			var previewImage = inforow.find('.ff_fileupload_preview_image');
		// 			previewImage.css('background-image', 'url(' + e.target.result + ')');
		// 			// Assuming fileInput is the file input element
		...
		// 				// Call DisplayPreviewDialog with the imgElement as the preview
		// 				DisplayPreviewDialog(imgElement, this, inforow, data, settings);
		// 			});
		// 		};
		// 		reader.onerror = function(e) {
		// 			console.error("FileReader error:", e); // Log any FileReader errors
		// 		};
		// 		reader.readAsDataURL(data.files[0]); // Read the file as a Data URL
		// 	}
		...
		return this.each(function() {
			...
			var dropzone = $('<input>').addClass('ff_fileupload_dropzone').attr({
				'type' : 'file',
				'name' : 'image',
				'id' : 'fileInput',
				'aria-label' : Translate('Browse, drag-and-drop, or paste files to upload'),
			});
			// var dropzone_2 = $('<input>').attr({
			// 	'type' : 'file',
			// 	'aria-label' : Translate('Browse, drag-and-drop, or paste files to upload'),
			// 	'name' : 'image',
			// });
			dropzonewrap.append(dropzone);
			// dropzonewrap.append(dropzone_2);
		...
		// // Remove the 'ff_fileupload_hidden' class from the input file with the ID 'fancy-file-upload'.
		// $('#fancy-file-upload').removeClass('ff_fileupload_hidden');
